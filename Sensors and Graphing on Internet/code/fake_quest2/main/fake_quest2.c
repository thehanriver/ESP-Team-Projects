...
#define R_ABOVE_DIVIDER 1000
#define R_BELOW_DIVIDER 660
#define GND_CALIBRATION -142
...
void init()
{
    ...
    printf("oscillator failed. exiting...\n");
    return;
    }
    ...
    printf("brightness failed. exiting...\n");
    return;
    }
}

static void right_align(char input[], int size, int width)
{
    char* copy = (char*) malloc(size);
    strncpy(copy, input, size);
    ...
    for (i=width-1; i>=0; i--)
    {
        ...
        {
            input[i] = copy[i-(width-len)];
        }
    }
}

static void my_alpha_display(char input[], int init) {
    int ret;

    // right align input
    right_align(input, 5, 4);
    // Write to characters to buffer
    uint16_t displaybuffer[4];
    displaybuffer[0] = alphafonttable[(int) input[0]];
    displaybuffer[1] = alphafonttable[(int) input[1]];
    displaybuffer[2] = alphafonttable[(int) input[2]];
    displaybuffer[3] = alphafonttable[(int) input[3]];
    ...
    if(ret == ESP_OK && init==0) {
      printf("Displayed: %c%c%c%c \n",input[0],input[1],input[2],input[3]);
    }
